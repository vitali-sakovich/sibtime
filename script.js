...
$(document).ready(function () {
    ...
    (function () {
        const btnCatalogDescShow = document.querySelector('.btn-catalog-desc__show');
    ...
    (function () {
        ...
        if (inputFile != null) {
            ...
            function cickCloseHeendler(e) {
                ...
                img.src = '/bitrix/templates/main/img/attach.svg';
            ...
            if (document.forms.reviews != null) {
                ...
                if (divFiles != null) {
                    ...
                    function eventChangeInput(e) {
                        if (e.target.tagName == 'INPUT') {
                            if (e.target.files != null) {
                                ...
                                let countFormFile = divFiles.querySelectorAll('.form_file').length;
                    ...
                    `;
                                /!* Add new block *!/
                                if (btnDelete == null) {
                                    divFiles.insertAdjacentHTML('beforeend', `
    ...
    (function () {
        const productCharacteristicsAdd = document.querySelector('.product-characteristics__add');
        const productCharacteristicsMin = document.querySelector('.product-characteristics__min');
        const productCharacteristicsAll = document.querySelector('.product-characteristics__all');
        ...
        productCharacteristicsAdd.addEventListener('click', function () {
            ...
            $('.product-characteristics__box').each(function( index ) {
                var list = $(this).find(".product-characteristics__list")
                var list = list.find(".product-characteristic__row").length
                if(list == 0){
                    $(this).hide()
                }
            });
    ...
    $('.tabs-btn').on('click', 'div:not(active)', function () {
        $(this)
            ...
            .closest('section.tabs-container').find('div.tab-content').removeClass('active').eq($(this).index()).addClass('active');
    });

    // табы
    $('.brands-sort').on('click', 'div:not(active)', function () {
        $(this)
            .addClass('active').siblings().removeClass('active')
            .closest('div.content_brands').find('div.tab-pane').removeClass('active').eq($(this).index()).addClass('active');
    });
    $('.dropdown-catalog__brands-types').on('click', 'div:not(active)', function () {
        $(this)
            .addClass('active').siblings().removeClass('active')
            .closest('div.dropdown-catalog__wrapper').find('div.dropdown-catalog__brands-container').removeClass('active').eq($(this).index()).addClass('active');
    });
    ...
    (function () {
        ...
        cardProductAll.forEach(function (cardProduct) {
            const cardProductImgHover = cardProduct.querySelector('.card-product__img-hover');
    ...
    (function () {
        ...
        const block = document.querySelector('.brands-logo');
        if (!brandsLogo) {
            return;
        }
    ...
    (function () {
        const collectionsAardShows = document.querySelector('.collections-card__shows');
...
function changeAddress() {
    ...
    profileCol.forEach(function (profileColItem) {
        ...
        const btnChangeAddress = profileColItem.querySelector('.btn-change-address');
...
const modalAuthorizationCode = document.querySelector('.modal-authorization-code');
const consentModal = document.querySelector('.consent-modal');
...
const madalBasket = document.querySelector('.basket');
...
const globalClickHandlers = {
    'js-open-modal': (node) => openModal(document.querySelector(node.dataset.openModal)),
}
...
//     const hoverImgAll = document.querySelectorAll('.js-hover-img');
//     const target = e.target;
//     Array.from(hoverImgAll).forEach(hoverImg => {
//         hoverImg.classList.remove('active');
//     })
...
window.addEventListener('load', function () {
    ...
    jQuery(function ($) {
        $.mask.definitions['~'] = '[1,2,3,4,5,6,9]';
        $('#soa-property-3, #soa-property-14').mask("+7 (~99) 999-99-99");
    });
...
function stageShows() {
    const filterBlock = document.querySelectorAll('.bx-filter-block');
    if(!filterBlock){
        return;
    }
    filterBlock.forEach((filterBlockItem)=>{
        const items = filterBlockItem.querySelectorAll('.checkbox');
        const div = document.createElement('div');
        const span = document.createElement('span');
        if(items.length > 7){
            div.textContent = 'Паказать все';
            span.textContent = '+';
            div.classList.add('add-spoiler');
            filterBlockItem.appendChild(div);
...
(() => {
    const contentCheckout = document.querySelector(".content_checkout-thank");
    if(!contentCheckout) return;
    const titleBlock = document.querySelector('.title-block');
    titleBlock.classList.add('title-block_checkout');
})();
...
const allFilters = () => {
    ...
    AllFilters.addEventListener('click', () => {
        if (AllFilters.textContent == 'Показать все фильтры') {
            AllFilters.textContent = 'Скрыть фильтры';
            document.querySelectorAll('.bx-filter-parameters-box').forEach((i) => {
                i.style.display = 'block';
            })
        } else {
            AllFilters.textContent = 'Показать все фильтры';
            document.querySelectorAll('.bx-filter-parameters-box').forEach((i) => {
...
function drawMap() {
    ...
    mapShop = new google.maps.Map(document.getElementById("map-shops"), myOptionsShops);
    var marker, i;
    var markerShops;
    var infowindow = new google.maps.InfoWindow();

    for (i = 0; i < locationsShops.length; i++) {
        var locationShop = locationsShops[i];
        var addressMarkerShops = locationShop.address;

        markerShops = new google.maps.Marker({
            position: new google.maps.LatLng(locationShop.lat, locationShop.lng),
            icon: {
                url: "<?=SITE_TEMPLATE_PATH?>/img/marker.svg",
                scaledSize: new google.maps.Size(43, 57)
            },
            map: mapShop,
        });

        google.maps.event.addListener(markerShops, 'mouseover', (function (markerShops, i, addressMarkerShops) {
            return function () {
                infowindow.setContent(addressMarkerShops);
                infowindow.open(mapShop, markerShops);
            }
        })(markerShops, i, addressMarkerShops));

        google.maps.event.addListener(mapShop, 'click', function () {
            infowindow.close();
        });
        google.maps.event.addListener(markerShops, 'mouseout', function () {
            infowindow.close();
        });
    }
